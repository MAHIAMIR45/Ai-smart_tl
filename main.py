...
# ---- Daily Trade Limits ----
DAILY_TRADE_MIN     = 2      # Minimum trades per day (very selective)
DAILY_TRADE_DEFAULT = 4      # Default max trades per day
DAILY_TRADE_HOT     = 8      # Max trades when market is very bullish
...
def _get_daily_limit() -> int:
    """Dynamic daily limit based on market condition."""
    label = sol_trend.get("label", "UNKNOWN")
    score = market_watch.get("market_score", 50)
    if "BULLISH" in label and score >= 75:
        return DAILY_TRADE_HOT
    elif "BEARISH" in label or score < 35:
        return DAILY_TRADE_MIN
    else:
        return DAILY_TRADE_DEFAULT

def _can_trade_today() -> bool:
    _check_daily_reset()
    limit = _get_daily_limit()
    if daily_trades >= limit:
        log_console(f"Daily limit reached: {daily_trades}/{limit} trades today", "WARN")
        return False
    return True
...
async function fetchData() {
  try {
    ...
    if (dtEl) dtEl.textContent = (d.daily_trades||0)+'/'+(d.daily_limit||'?')+' trades';
    ...
    if (dlEl) dlEl.textContent = 'Daily limit: '+(d.daily_limit||'?')+' trades today';
...
Today's trades: {daily_trades}/{_get_daily_limit()} | Today's fees: ${daily_fees:.4f}
...
💸 Fees: ${total_fees_paid:.4f} | Today: {daily_trades}/{_get_daily_limit()}
...
Bot Balance: ${balance_usd:.2f} | Daily trades: {daily_trades}/{_get_daily_limit()}
...
📅 Today: {daily_trades}/{_get_daily_limit()} trades""")
    return True
...
def main_loop():
    ...
    log_console(f"Fee: {DEX_FEE_PCT+SLIPPAGE_PCT:.2f}% round-trip | Daily limit: {DAILY_TRADE_DEFAULT} (up to {DAILY_TRADE_HOT} in hot market)", "SYSTEM")
...
Daily limit: {DAILY_TRADE_DEFAULT}-{DAILY_TRADE_HOT} trades
...
/status /balance /pause /help""")
    ...
    while True:
        ...
        log_console(
            ...
            f"{ai_mode} | Trades:{daily_trades}/{daily_limit} | Score:{market_score}% | Paused:{bot_paused}",
            "SYSTEM"
        )
        ...
        if can_trade:
            ...
            if "BEARISH" in trend_label and market_score < 45:
                log_console(
                    f"🐻 BEARISH + low score ({market_score}%) — skipping new buys",
                    "WARN"
                )
            else:
                pairs = get_dexscreener_pairs()
                log_console(f"Scanning {len(pairs)} pairs | {trend_label} | Score:{market_score}%", "INFO")
                for pair in pairs:
            ...
            if bot_paused:
                ...
                why = f"daily limit {daily_trades}/{daily_limit}"
            else:
                why = "low balance"
